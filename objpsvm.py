...
def svm_objective_train(data, labels):
    '''
    objective peturbation
    support vector machine
    '''
    c = 1/(2*parameters()[2])#chaudhuri2011differentially corollary 13
    ...
    Epsilonp = parameters()[0]-2*log(1+c/(parameters()[1]*L))
    if Epsilonp>0:
        Delta = 0
    else:
        Delta = c/(L*(exp(parameters()[0]/4)-1))-parameters()[1]
        Epsilonp = parameters()[0]/2
    scale = Epsilonp/2
    ...
    def obj_func(x):
        ...
        f = (1/L)*jfd + (1/2)*parameters()[1]*(norm(x)**2) + (1/L)*dot(noise,x) + (1/2)*Delta*(norm(x)**2)
        return f
